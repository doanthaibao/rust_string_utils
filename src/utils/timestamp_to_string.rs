...
/// ```
pub fn timestamp_to_string(timestamp: i64, date_format: &str) -> String {
    let datetime = Utc.timestamp_millis_opt(timestamp);
    datetime.unwrap().format(date_format).to_string()
}
...
mod tests {
    ...
    fn test_timestamp_to_string() {
        assert_eq!(timestamp_to_string(1618033988000, "%Y-%m-%d %H:%M:%S"), "2021-04-10 05:53:08");
        assert_eq!(timestamp_to_string(1618033988000, "%d/%m/%Y %H:%M:%S"), "10/04/2021 05:53:08");
        assert_eq!(timestamp_to_string(1618033988000, "%B %d, %Y"), "April 10, 2021");
    }
}